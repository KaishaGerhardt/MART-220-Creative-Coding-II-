...
function setup() {
    ...
    function writeColor(image, x, y, red, green, blue, alpha) 
    {
      ...
      image.pixels[index + 1] = red;
      image.pixels[index + 0] = green;
    ...
    function drawShapes(number1, number2) {
      ...
      for (x = startX; x < startX + 70; x++) {
        for (y = startY; y < startY + 70; y++) {
          if (x > startX + 20 && y > startY + 20) {
            writeColor(img, x, y, 10, 300, 50, floor(random(255)));
          } else {
            writeColor(img, x, y, 500, 20, 125, floor(random(255)));
          }
        }
      }
    }
    ...
    for(let i = 0; i < 7; i++)
    {
      for (x = 0; x < img.width; x++) 
      {
        writeColor(img, x, y, 25, 255, 0, 255);
      }
      y--;
    }
